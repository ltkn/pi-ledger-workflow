...
- Put the tests ONLY in NEW test files, never in an existing file: a copy of an existing file would overwrite what the workers change in it, so the harness drops it. Save each file at ${S}/<task id>/<the path it will have in the repository>, e.g. ${S}/T2/src/test/java/com/acme/order/OrderCancelTest.java. The harness copies the task's files into place when the task starts.
- When the plan says to change or extend existing tests, leave those edits to the workers. Write the acceptance checks for the new behaviour in a new file next to the existing one, named after it plus the behaviour it pins and a "Spec" suffix (e.g. OrderRenderTest → OrderRenderCompactHeaderSpecTest, test_order.py → test_order_compact_header_spec.py), reusing its helpers where they are accessible. The behaviour in the name keeps it unique: earlier features may already have left Spec files there, and those are existing files too. Never put more than one task's tests in the same file.
